        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="支付状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-dict-select-tag type="list" v-decorator="['payStatus']" :trigger-change="true" dictCode="pay_status" placeholder="请选择支付状态" />
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'createTime','customer','orderNo','orderTotal','driver','carNo','phone','payStatus'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'createTime','customer','orderNo','orderTotal','driver','carNo','phone','payStatus'))
      },
